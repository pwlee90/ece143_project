...
def merge_src_files() -> pd.DataFrame:
    ...
    albums = pd.read_csv("data/spotify_albums.csv") # needs id to be renamed to album_id
    artists = pd.read_csv("data/spotify_artists.csv") # needs id to be renamed to artist_id
    ...
    merged_data = reduce(
        lambda left, right: pd.merge(left, right, on="track_id", how="inner"), [tracks, albums, artists, audio, lyrics]
    )
...
def extract_datetime(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fully_specified_dates = df[df["release_date_precision"] == "day"]
    fully_specified_dates["release_date"] = pd.to_datetime(fully_specified_dates["release_date"], format="%Y-%m-%d")
...
def extract_genres(df : pd.DataFrame) -> pd.DataFrame:
    ...
    def genre_string_to_list(genre_string):
        return [genres.replace("'", "").replace("]", "").replace("[", "").strip() for genres in genre_string.split(",")]
    
    df["genres"] = df["genres"].apply(lambda genre_string : genre_string_to_list(genre_string))
    return df

if __name__ == "__main__":
    merged_data = merge_src_files()
    cleaned_data = drop_irrelevant_columns(merged_data)
    cleaned_data = extract_datetime(cleaned_data)
    cleaned_data = extract_genres(cleaned_data)
